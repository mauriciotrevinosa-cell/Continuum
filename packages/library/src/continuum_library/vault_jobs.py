    ...
    for root_key in root_keys:
        ...
        if hash_after:
            hashed, _ = enqueue(
                ...
                priority=1,
                max_attempts=5,
                depends_on=[scan.id] if created else None,
            )
...
def request_import(session: Session, root_key: str) -> Job:
    job, _ = enqueue(session, INTAKE_IMPORT_JOB, payload={"root_key": root_key}, priority=3)
    session.flush()
    return job
